...
namespace cgx::render
{
    ...
    std::shared_ptr<Mesh> ResourceManager::getMesh(const std::string& meshID)
    {
        ...
        {
            // todo: return a placeholder mesh
    ...
    std::shared_ptr<Shader> ResourceManager::loadShader(const std::string& name, const std::string& shader_dir)
    {
        ...
        m_shaders[name] = std::make_shared<cgx::render::Shader>(name, shader_dir, vert_path, frag_path);
    ...
    std::shared_ptr<Model> ResourceManager::loadModel(const std::string& path)
    {
        ...
        for (auto& [material_id, vertices] : material_to_vertices) 
        {
            ...
            std::shared_ptr<cgx::render::Material> material = m_materials[material_id_str];

            std::vector<unsigned int>& indices = material_to_indices[material_id];
            
            auto mesh = std::make_shared<cgx::render::Mesh>(vertices, indices, material);
